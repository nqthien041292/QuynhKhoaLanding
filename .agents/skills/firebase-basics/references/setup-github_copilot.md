...
To get the most out of Firebase with GitHub Copilot in VS Code, follow these steps to install the agent skills and the
MCP server.
...
1. **Check Local skills**: Run `npx skills list --agent github-copilot`. If the output includes `firebase-basics`, the
   skills are already installed locally.
...
1. **Locate `mcp.json`**: Find the configuration file for your environment:
   ...
   *Note: If the `.vscode/` directory or `mcp.json` file does not exist, create them and initialize the file
   with `{ "mcp": { "servers": {} } }` before proceeding.*
...
3. **Add or Update Configuration**: If the `firebase` block is missing or incorrect, add it to the `mcp.servers` object:
   ...
   ```
   *CRITICAL: Merge this configuration into the existing `mcp.json` file under the `mcp.servers` object. You MUST
   preserve any other existing servers inside `mcp.servers`.*
...
1. **Restart VS Code**: Instruct the user to restart VS Code. **Stop and wait** for their confirmation before
   proceeding.
2. **Confirm Connection**: Check the MCP server list in the VS Code Copilot UI to confirm that the Firebase MCP server
   is connected.
