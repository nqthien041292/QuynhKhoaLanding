...
1. **Check Local skills**: Run `npx skills list --agent cursor`. If the output includes `firebase-basics`, the skills
...
1. **Locate `mcp.json`**: Find the configuration file for your operating system:
   ...
   *Note: If the directory or `mcp.json` file does not exist, create them and initialize the file
...
3. **Add or Update Configuration**: If the `firebase` block is missing or incorrect, add it to the `mcpServers` object:
   ...
   ```
   *CRITICAL: Merge this configuration into the existing `mcp.json` file. You MUST preserve any other existing servers
...
1. **Restart Cursor**: Instruct the user to restart the Cursor application. **Stop and wait** for their confirmation
   before proceeding.
2. **Confirm Connection**: Check the MCP server list in the Cursor UI to confirm that the Firebase MCP server is
   connected.
