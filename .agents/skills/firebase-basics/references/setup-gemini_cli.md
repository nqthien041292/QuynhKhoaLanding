...
1. **Check Existing Extensions**: Run `gemini extensions list`. If the output includes `firebase`, the extension is
   already installed.
...
1. **Restart Gemini CLI**: Instruct the user to restart the Gemini CLI if any new installation occurred. **Stop and wait
...
1. **Check Existing Configuration**: Run `gemini mcp list`. If the output includes `firebase-tools`, the MCP server is
   already configured.
...
1. **Restart Gemini CLI**: Instruct the user to restart the Gemini CLI. **Stop and wait** for their confirmation before
   proceeding.
